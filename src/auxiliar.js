...
var InputServer = function() {
	...
	var clientToCard = function(x,y) {
		...
		game.onClick(card); //tendremos que implementar la funcion onClick
	}; //Transforma las pulsaciones de raton a cual es la carta que se esta pulsando

	/*Aqui nos encargamos de que el servidor se quede a la escucha de los eventos y haga la transformacion de coordenada a carta
	Hay eventos de touch y click (independientes). Desde una tactil se lanzan eventos de touch y click (el de click tiene un pequeño
	delay), luego no podemos usarlo bien. Lo que hay a continuacion es que si recibes un touch, desactivas el click. Si recibe uno de
	click y nunca uno de touch, simplemente no hara el touch*/
